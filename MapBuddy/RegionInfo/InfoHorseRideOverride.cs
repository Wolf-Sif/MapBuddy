...
{
    internal class InfoHorseRideOverride
    {
        Logger logger = new Logger();

        string output_dir = "";
        string header = "";
        string propertyName = "HorseRideOverride";
        string combined_file_name = "";
        string file_format = "csv";

        public InfoHorseRideOverride(string path, Dictionary<string, string> mapDict, bool splitByMap)
        {
        ...
        public void WriteCSV(MSBE msb, string map_name, bool outputByMap)
        {
            ...
            foreach (MSBE.Region.HorseRideOverride entry in msb.Regions.HorseRideOverrides)
            {
                string line = "";
